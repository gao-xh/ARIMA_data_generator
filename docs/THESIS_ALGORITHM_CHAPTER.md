...
2.  **效期双重保险机制 (Dual-Mechanism Validity Control)**: 
    提出了基于“名义效期预测”与“有效库存扣除”相结合的策略。
    *   **保供优先**: 预测时假设补货为新鲜批次，避免因现有库存临期而错误降低补货目标（解决了“恐慌性去库存”导致的缺货问题）。
    *   **风险对冲**: 在计算净需求时，提前扣除在补货周期内即将过期的库存 ($I_{eff} = I_{total} - I_{expiring}$)，确保补货量覆盖预期损耗。
